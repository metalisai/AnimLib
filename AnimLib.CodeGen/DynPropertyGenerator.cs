...
public class DynPropertyGenerator : IIncrementalGenerator
{
        ...
        context.RegisterSourceOutput(combined, (spc, source) =>
        {
            ...
            foreach (var dclass in classes)
            {
                var classFields = fields!
                    ...
                    .ToList();
                var code = GenerateDynPropertiesCode(classFields, dclass!.forTypeName, dclass!.stateClassName, dclass!.onlyProperties);
    ...
    private static DynClassInfo? GetDynClassInfo(GeneratorSyntaxContext context)
    {
        ...
        foreach (var attr in symbol.GetAttributes())
        {
            if (attr.AttributeClass?.ToDisplayString() == "GenerateDynPropertiesAttribute")
            {
                ...
                return new DynClassInfo(forType.ToDisplayString(), forType.Name, stateClassName, generateOnlyProperties);
            }
        }
    ...
    private static void GenerateDynMethods(IEnumerable<DynFieldInfo?> fields, string forClassName, string stateClassName, StringBuilder sb)
    {
        // GetState to a state object
        sb.AppendLine($"        private protected void GetState({stateClassName} state, Func<DynPropertyId, object?> evaluator) {{");
        ...
        // Copy constructor
        sb.AppendLine($"        internal {forClassName}({forClassName} other) : base(other) {{");
        foreach (var field in fields)
        {
            if (field == null) continue;
            var (_, backingFieldName) = GetPropertyNames(field);
        ...
        sb.AppendLine($"            return new {forClassName}(this);");
        sb.AppendLine("        }"); // Clone
    }

    private static string GenerateDynPropertiesCode(IEnumerable<DynFieldInfo?> fields, string forClassName, string stateClassName, bool onlyProperties)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine($"    public partial class {forClassName}");
        sb.AppendLine("    {");
        ...
        if (!onlyProperties)
        {
            GenerateDynMethods(fields, forClassName, stateClassName, sb);
        }
    ...
    private record DynClassInfo(string forFullTypeName, string forTypeName, string stateClassName, bool onlyProperties);
}
