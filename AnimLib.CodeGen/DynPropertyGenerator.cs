...
public class DynPropertyGenerator : IIncrementalGenerator
{
        ...
        context.RegisterSourceOutput(combined, (spc, source) =>
        {
            ...
            foreach (var dclass in classes)
            {
                var classFields = fields!
                    ...
                    .ToList();
                var code = GenerateDynPropertiesCode(classFields, dclass!.forTypeName, dclass!.stateClassName);
    ...
    private static DynClassInfo? GetDynClassInfo(GeneratorSyntaxContext context)
    {
        ...
        foreach (var attr in symbol.GetAttributes())
        {
            if (attr.AttributeClass?.ToDisplayString() == "GenerateDynPropertiesAttribute")
            {
                ...
                return new DynClassInfo(forType.ToDisplayString(), forType.Name, stateClassName);
            }
        }
    ...
    private static DynFieldInfo? GetDynFields(GeneratorSyntaxContext context)
    {
        ...
        return new DynFieldInfo(className, ns, fieldName, fieldType, initializer);
    }
    ...
    private static string GenerateDynPropertiesCode(IEnumerable<DynFieldInfo?> fields, string forClassName, string stateClassName)
    {
    ...
    private record DynFieldInfo(string ClassName, string Namespace, string FieldName, string Type, string Initializer);
    private record DynClassInfo(string forFullTypeName, string forTypeName, string stateClassName);
}
