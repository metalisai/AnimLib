...
vec4 filterPix(vec4 inputColor) {
    vec3 filtered = max(inputColor.rgb-vec3(_BloomThreshold), vec3(0.0));
    // calculate luminance
...
void main() {
    ...
    vec4 srcColor = filterPix(texture(_MainTex, texCoord, 0));
    ...
    for(int j = -steps+1; j < steps; j++) {
        ...
        vec4 sample = filterPix(texture(_MainTex, texCoord + offset, 0));
