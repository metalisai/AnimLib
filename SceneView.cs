...
namespace AnimLib {
    ...
    public class SceneView
    {
        ...
        public SceneView(RenderBuffer rb, int x, int y, int width, int height)
        {
            renderBuffer = rb;
            this.x = x;
            this.y = y;
        ...
        {
            renderBuffer = new DepthPeelRenderBuffer();
            renderBuffer.Resize(bufferWidth, bufferHeight);
            this.x = x;
            this.y = y;
            this.width = width;
            this.height = height;
        ...
        public TransformResult? DoSelectionGizmo(Vector3 pos, Quaternion rot, Vector3 scale) {
            ...
            if (cam != null) {
                ...
                var projM = cam.CreateViewToClipMatrix((float)renderBuffer.Width/(float)renderBuffer.Height);
        ...
        public OrthoResult? DoOrthoGizmo(Vector2 pos, float rotation) {
            ...
            var projM = M4x4.Ortho(0.0f, renderBuffer.Width, 0.0f, renderBuffer.Height, -1.0f, 1.0f);
        ...
        protected Vector2 bufferToScreen(Vector2 bufP) {
            ...
            var bufferSize = new Vector2(renderBuffer.Width, renderBuffer.Height);
        ...
        protected Vector2? worldToBuffer(CameraState cam, Vector3 pos) {
            var w2c = cam.CreateWorldToClipMatrix((float)renderBuffer.Width/renderBuffer.Height);
            var clipPos = w2c * new Vector4(pos, 1.0f);
            float x = ((clipPos.x/clipPos.w)*0.5f + 0.5f) * renderBuffer.Width;
            float y = ((clipPos.y/clipPos.w)*0.5f + 0.5f) * renderBuffer.Height;
            y = renderBuffer.Height - y;
            var ret = clipPos.w > 0.0f ? new Vector2(x, y) : (Vector2?)null;
            if (ret == null || (ret.Value.x < 0.0f || ret.Value.x > renderBuffer.Width || ret.Value.y < 0.0f || ret.Value.y > renderBuffer.Height))
                return null;
            return ret;
        }
        ...
        public CapturedFrame CaptureScene()
        {
            ...
            var tex = new CapturedFrame(renderBuffer.Width, renderBuffer.Height, Texture2D.TextureFormat.RGB8);
            renderBuffer.Bind();
